  ...
  return (
      <div className="carrousel overflow-hidden ">
        <div className="box" style={{transform: `perspective(4000px) rotatey(${deg}deg)`}}>
          ...
          {tab.map((t, k) => <span ref={test} key={k} className={`${actif ? 'actif' : 'passif'}`}
                                   style={{transform: `rotateY(${(max / ar) * (k)}deg) translatez(${z}px)`}}>
        ...
        <div onClick={handleClickLeft} className="left"><i className="far fa-arrow-alt-circle-left text-white"></i>
        </div>
        <div onClick={handleClickRight} className="right"><i className="far fa-arrow-alt-circle-right text-white"></i>
        </div>
      </div>
  );
}
